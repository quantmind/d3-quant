...
export default function (data, opts) {
  ...
  if (!dimensions.length) throw new Error("No dimensions");
  ...
  if (opts && opts.crossfilter) {
    ...
    return s;
  } else return serie(dserie.data).labels(dserie.labels);
}
...
function fillDimension(values, dserie, dimensions, index, entry0) {
  ...
  dim.forEach(function (e) {
    entry = entry0.slice();
    entry.push(e);
    if (index < N) fillDimension(values, dserie, dimensions, index + 1, entry);
    else {
